"""The Home dashboard: one card per tool, with its recent folders."""
...
class ToolCard(QFrame):
    openRequested = Signal(str)                  # tool id
    ...
    def __init__(self, spec, shortcut, parent=None):
        ...
        version = QLabel("v%s" % spec.version)
        version.setObjectName("Chip")
        top.addWidget(version, 0, Qt.AlignmentFlag.AlignTop)
        ...
        self.open_button = QPushButton("Open %s" % spec.name)
        ...
        key.setToolTip("Open %s from anywhere in the suite" % spec.name)
        buttons.addWidget(key)
        layout.addLayout(buttons)

    def set_theme(self, theme) -> None:
        self.mark.setPixmap(icons.mark_pixmap(self.spec.mark, theme["accent"],
                                              theme["surfaceAlt"], 52))
    ...
    def set_recent(self, folders) -> None:
        ...
        for folder in folders[:3]:
            link = QPushButton(metrics.elidedText(folder, Qt.TextElideMode.ElideMiddle, 420))
            link.setObjectName("Link")
            link.setToolTip(folder)
            link.setCursor(Qt.CursorShape.PointingHandCursor)
            link.clicked.connect(lambda _c=False, f=folder:
                                 self.folderRequested.emit(self.spec.id, f))
...
class HomePage(QWidget):
    ...
    def __init__(self, tools, parent=None):
        ...
        centre.setContentsMargins(24, 28, 24, 28)
        centre.addStretch(1)
        column = QWidget()
        ...
        self.hero_mark.setFixedSize(64, 64)
        ...
        body.addWidget(section_label("Tools"))
        ...
        for number, spec in enumerate(self.tools, start=1):
            card = ToolCard(spec, "Ctrl+%d" % number)
        ...
        ghost_layout.setContentsMargins(22, 20, 22, 20)
        ghost_title = QLabel("More tools")
        ghost_title.setObjectName("CardTitle")
        ghost_text = QLabel("New tools appear here as they join the suite, and share "
                            "its theme, shortcuts and safe-saving.")
        ...
        footer = QLabel("Ctrl+Shift+H comes back here from any tool  ·  "
                        "Ctrl+Q quits  ·  %s %s" % (SUITE_NAME, SUITE_VERSION))
    ...
    def set_theme(self, theme) -> None:
        self.hero_mark.setPixmap(icons.mark_pixmap("suite", theme["accent"],
                                                   theme["surfaceAlt"], 64))
    ...
    def _reflow(self, force=False) -> None:
        ...
        for widget in self.cards + [self.ghost]:
            self.grid.removeWidget(widget)
        for position, widget in enumerate(self.cards + [self.ghost]):
