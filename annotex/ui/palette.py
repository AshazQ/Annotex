...
def _derive(t: dict) -> dict:
    ...
    t.setdefault("onDanger", readable_on(t["danger"]))
    t.setdefault("dangerHover", mix(t["danger"], "#000000", 0.88))
...
def stylesheet(t: dict) -> str:
    """One stylesheet for the whole suite - built in annotex.ui.style from the
    theme's colours and the design system's sizes."""
    from .style import build
    return build(t)
